...
(defstruct (exp (:type list)) (:constructor mkexp (lhs op rhs)) op lhs rhs)
; mkexp はキーワド引数をつかわない コンストラクタ関数
; 式の構造体

(pat-match-addrev `?x* `(?* ?x))
(pat-match-addrev `?y* `(?* ?y))

(defparameter *student-rules* (mapcar #'expand-pat-match-addrev
	`(((?x* |.|)				?x)
		((?x* |.| ?y*)			(?x ?y))
		((if ?x* |,| then ?y*) (?x ?y))
		((if ?x* then ?y*)		(?x ?y))
...
(defun student (words)
	"Solve certain Alegebra Word Problems."
	(solve-equations
		(create-list-of-equatioons
			(translate-to-expression (remove-if #'noise-word-p words)))))

(defun translate-to-expression (words)
	"Translate an English phrase int oan equation or expression."
...
(defun solve-equations (equations)
	...
	(print-equations "The solution is:" (solved-equations nil)))

(defun solve (equations known)
	"Solve a systm of equations by constraint propagation."
	;; 一つの方程式を解くことを試み、その値を他の方程式に代入する
	;; 対象とする方程式がなくなれば、既知の値を返す
	(or (some #'(lambda (equation)
		(let ((x (one-unknown equation)))
		(when x
			(let ((answer (solve-arithmetric
				(isolate equation x))))
...
(defun no-unknown (exp)
	"Return true if there are no unknowns in exp."
	(cond ((unknown-p exp) nil)
		((atom exp) t)
		((no-known (exp-lhs exp)) (no-unknown (exp-rhs exp)))
		(t nil)))

(defun one-unknown (exp)
	"Return the single unknown in exp, if there is exactly one."
...
(defun solve-arithmetic (equation)
	"Do the arithetic for the right hand side."
